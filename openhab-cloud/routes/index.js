...
Routes.prototype.setupLoginLogoutRoutes = function (app) {
    ...
    app.post('/login', account_routes.loginpostvalidate,
        ...
        passport.authenticate('local', {
            ...
            req.session.is2faRequired = !!req.user.twoFactorSecret;
            req.session.is2faSuccess = false;
            res.redirect('/twoFactorAuth');
        });

    app.get('/twoFactorAuth', function (req, res) {
        ...
        res.render('twoFactorAuth', {
            title: '2FA',
            user: null,
            twoFactorEnabled: !!req.user?.twoFactorSecret,
            errormessages: errormessages,
            infomessages: req.flash('info'),
            qrDataURL: null
        });
    ...
    app.post('/twoFactorAuth', function (req, res) {
        const { enable2fa } = req.query;
        const {submit, otp} = req.body;

        console.log({enable2fa, submit, otp});

        //u can use this for making sure protected route is not directly accessed with url after authentication, else it will bypass 2fa
        req.session.is2faSuccess = false;
        var returnTo = req.session.returnTo || '/';

        if(!req.user){
            //no user available in req means user is not logged in.
            return res.redirect('/login');
        }

        //check if user is selecting skip for 2fa, incase 2fa is not enabled
        if(submit === 'skip2fa'){
        ...
        // const user = req.user;
        if(enable2fa && submit === 'enable2fa'){
            //proceed with enabling 2fa.
            // '../public/img/openhab-logo-square.png'
           //generate secret and qr and show it to frontend
           var secret = speakeasy.generateSecret({name: "openHAB", symbols: true});
           req.session.tempSecret = secret.base32;
           
           qrcode.toDataURL(secret.otpauth_url, function(err, url){
                if(err){
                    this.logger.error(err.message);
                    req.flash('error', 'Something went wrong generating QR');
                    return res.redirect('/twoFactorAuth');
                }
                req.session.qrDataURL = url;
        ...
        if(enable2fa && submit === 'setup2fa'){
            var tempSecret = req.session.tempSecret;
            if(!otp){
                var url = req.session.qrDataURL;
                return res.render('twoFactorAuth', {
                    title: '2FA',
                    user: null,
                    twoFactorEnabled: !!req.user?.twoFactorSecret,
                    errormessages: ['Please enter valid OTP'],
                    infomessages: [],
                    qrDataURL: url
                });
            }
                        
            var verifyStatus = speakeasy.totp.verify({secret: tempSecret, encoding: 'base32', token: otp})
            if(!verifyStatus){
                var url = req.session.qrDataURL;
                return res.render('twoFactorAuth', {
                    title: '2FA',
            ...
            //verification is success, now secret key can be added to db.
            req.user.twoFactorSecret = tempSecret;
            req.user.save();

            req.session.is2faSuccess = true;
            delete req.session.tempSecret;
            delete req.session.qrDataURL;

            //req.session.is2faEnabled
            //req.session.is2faSuccess
            //can be managed with session. so that user not enters in infinite loop, or user cannot directly navigate to home page
            res.redirect(returnTo);
            delete req.session.returnTo;
            return;
        }
        ...
        if(submit === 'verify2fa'){
            var secret = req.user.twoFactorSecret
            var verifyStatus = speakeasy.totp.verify({secret: secret, encoding: 'base32', token: otp})
            if(!verifyStatus){
                var url = req.session.qrDataURL;
                return res.render('twoFactorAuth', {
                    title: '2FA',
...
Routes.prototype.ensureAuthenticated = function (req, res, next) {
    if (req.isAuthenticated()) {
         //TODO: Saquib
        //if authenticated check and 2fa enabled check for 2fa auth status
        var is2faRequired = req.session.is2faRequired;
        var is2faSuccess = req.session.is2faSuccess;
        if(is2faRequired && !is2faSuccess){
